...
→ connected stations form a power grid.
...
1. If x is online → the check is resolved by x itself.


2. If x is offline → it’s handled by the smallest online station ID in the same grid.


3. If no online station exists → return -1.
...
All stations start online → one big grid {1,2,3,4,5}

[1,3] → station 3 online → answer = 3

[2,1] → 1 goes offline

[1,1] → 1 offline → smallest online in grid = 2

[2,2] → 2 goes offline

[1,2] → 2 offline → next online = 3
...
Each [1, x] query may traverse the whole grid → O(N + E)

Too slow for 10⁵ stations.
...
comp_min[root] → smallest currently online station in that component.

next_node[i] → next station (by ID) in same component.
...
This way, each station is “skipped” at most once → near-linear time overall.
...
Each node is walked past at most once during pointer advancement → effectively linear.
...
1. [1,1] → 1 online → answer = 1


2. [2,1] → 1 offline


3. [1,1] → 1 offline, no online node → -1
...
The smallest ideas — linking and skipping — can replace heaps, sets, or sorts.
...
> ⚡ In short:
Don’t overcomplicate the grid.
Each component already knows its smallest beating heart —
we just listen, then move to the next when it goes silent. 🤍